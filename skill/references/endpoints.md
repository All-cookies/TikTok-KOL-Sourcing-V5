...
V2 版本新增多维筛选功能，需要计算互动率：

**计算公式**：
```
互动率 = (点赞数 + 评论数) / 粉丝数 × 100%
       = (best_video_likes + best_video_comments) / follower_count × 100%
```

**数据来源**：
- 点赞数：`aweme_info.statistics.digg_count`
- 评论数：`aweme_info.statistics.comment_count`
- 粉丝数：`author.follower_count`

**注意**：如果 API 未返回评论数，可以只使用点赞数估算。

---

## V2 新增：国家/地区推断

TikHub API 返回的地理位置字段可能不完整，需要从以下数据推断：

1. **bio 语言**：通过 bio 文本判断语言，间接推断地区
2. **视频内容**：某些国家特有的内容模式（如特定节日、活动）
3. **评论区域**：视频评论区的主要语言

**简化处理**：默认返回空，需要用户在筛选时手动标记。