...
在生成脚本前，必须从 Phase 2.5 的用户偏好中获取以下参数：

| 参数 | 来源 | 说明 |
|------|------|------|
| `FOLLOWER_MIN` | 用户指定 | 最小粉丝数（例：3000） |
| `FOLLOWER_MAX` | 用户指定 | 最大粉丝数（例：200000） |
| `TARGET_MARKET` | 用户指定 | 目标市场（例：US） |

**这些参数必须正确设置，否则采集结果不符合用户需求**

---

## 自定义区（必须根据 Phase 2.5 修改）

```typescript
// ============================================================
// CUSTOMIZE: 必须根据 Phase 2.5 用户偏好设置
// ============================================================
...
// ⚠️ 以下参数必须从 Phase 2.5 获取：
const FOLLOWER_MIN = 3000;   // 用户指定的最小粉丝数
const FOLLOWER_MAX = 200000;  // 用户指定的最大粉丝数

const CATEGORY_KEYWORDS = ["cat litter", "litter box", "cat care", "pet tech", "smart pet"];
const COMPETITOR_KEYWORDS = ["litter robot", "whisker", "petmate"];
const OUTPUT_LABEL = "petkit-cat-litter";

// 不需要设置 TARGET_TOTAL，让脚本输出全部符合条件的数据
```
...
1. **移除了 `TARGET_TOTAL` 限制** — V2 按需采集，不人为截断
2. **FOLLOWER_MIN/MAX 来自用户偏好** — 不再使用默认值
3. **移除了 TOP N 截断逻辑** — 全部输出符合条件的数据
...
// ⚠️ 以下参数必须从 Phase 2.5 用户偏好获取：
const FOLLOWER_MIN = 3000;   // 例：用户说 3K-200K
const FOLLOWER_MAX = 200000; // 例：用户说 3K-200K
const CATEGORY_KEYWORDS = ["cat litter", "litter box", "cat care", "pet tech", "smart pet"];
const COMPETITOR_KEYWORDS = ["litter robot", "whisker", "petmate"];
const OUTPUT_LABEL = "petkit-cat-litter";
...
async function searchByVideos(): Promise<Map<string, Creator>> {
  ...
  for (const task of SEARCH_TASKS) {
    ...
    console.log(`   粉丝区间: ${FOLLOWER_MIN.toLocaleString()} - ${FOLLOWER_MAX.toLocaleString()}`);
    let offset = 0;
    let pageCount = 0;
    ...
    while (pageCount < MAX_PAGES) {
      try {
        ...
        for (const item of items) {
          ...
          // ⚠️ 使用 Phase 2.5 用户偏好的粉丝区间过滤
          if (follower < FOLLOWER_MIN || follower > FOLLOWER_MAX) continue;
...
<title>TikTok KOL Report — ${escHtml(OUTPUT_LABEL)} | ${FOLLOWER_MIN/1000}K-${FOLLOWER_MAX/1000}K</title>
<style>
...
<div class="container">
  <header>
    ...
    <div class="meta">${escHtml(OUTPUT_LABEL)} | 粉丝 ${(FOLLOWER_MIN/1000).toFixed(0)}K-${(FOLLOWER_MAX/1000).toFixed(0)}K | ${timestamp}</div>
  </header>
  <div class="cards">
    ...
    <div class="card"><div class="label">B 级（值得联系）</div><div class="value orange">${tierB}</div></div>
...
async function main() {
  console.log(`🚀 开始红人采集（粉丝区间: ${FOLLOWER_MIN.toLocaleString()}-${FOLLOWER_MAX.toLocaleString()}）...\n`);

  // Phase 1: 视频搜索发现创作者（使用 Phase 2.5 用户偏好的粉丝区间）
  ...
  // ⚠️ 移除了 .slice(0, TARGET_TOTAL)，输出全部数据
  ...
  console.log(`📊 用户偏好：粉丝 ${FOLLOWER_MIN.toLocaleString()}-${FOLLOWER_MAX.toLocaleString()}`);
  console.log("📊 采集摘要：");
  for (const [kw, count] of kwStats) {
    console.log(`   "${kw}": ${count} 位博主`);
  }
  console.log(`   合计：${scored.length} 位博主\n`);
  console.log("📋 建联分层：");
  console.log(`   A 级（优先建联）: ${tierA} 位`);
  console.log(`   B 级（值得联系）: ${tierB} 位`);
  console.log(`   C 级（备选观察）: ${tierC} 位\n`);
  console.log(`📧 有邮箱：${withEmail} 位（${scored.length > 0 ? ((withEmail / scored.length) * 100).toFixed(1) : 0}%）`);
...
1. **必须先完成 Phase 2.5** — 获取用户偏好的粉丝区间
2. Claude 复制此模板，修改 `CUSTOMIZE` 区域的常量
3. 将脚本保存为临时文件（如 `_scrape.ts`）
4. 用 `npx tsx _scrape.ts` 执行
5. 执行完毕后删除临时脚本文件
6. CSV + HTML 报告输出在 `output/` 目录（可直接双击 `.html` 文件在浏览器中查看）
7. 每次执行后自动更新 `output/index.html` —— 采集中心首页，列出所有历史搜索记录

## V2 变更说明

| 变更项 | V1 | V2 |
|--------|----|----|
| TARGET_TOTAL | 50（固定） | 无限制 |
| Top N 截断 | 有 | 无（全部输出） |
| FOLLOWER_MIN/MAX | 默认值 5000/5000000 | 来自 Phase 2.5 用户偏好 |
| 输出数据量 | 固定 50 条 | 根据实际采集 |
